...
from ezmsg.sigproc.base import BaseTransformerUnit, processor_state, BaseStatefulTransformer
...
class DiffTransformer(BaseStatefulTransformer[DiffSettings, AxisArray, AxisArray, DiffState]):
    ...
    def _process(self, message: AxisArray) -> AxisArray:
        ...
        diffs = np.diff(
            np.concatenate((self.state.last_dat, message.data), axis=ax_idx), axis=ax_idx
        )
...
def diff(
    axis: str = "time", scale_by_fs: bool = False
) -> DiffTransformer:
