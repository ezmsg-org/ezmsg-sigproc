...
class ResampleState(ez.State):
    ...
    ref_axis: tuple[typing.Union[AxisArray.TimeAxis, AxisArray.CoordinateAxis], int] = (AxisArray.TimeAxis(fs=1.0), 0)
...
class ResampleProcessor(
    ...
    def reset(self, message: AxisArray) -> None:
        ...
        ax_idx: int = message.get_axis_idx(self.settings.axis)
        ax = message.axes[self.settings.axis]
        in_tvec = ax.data if hasattr(ax, "data") else ax.value(np.arange(in_dat.shape[ax_idx]))
        in_dat = message.data
        if ax_idx != 0:
        ...
        if self.settings.resample_rate is None:
            ...
            out_axes = {k: v for k, v in message.axes.items() if k != self.settings.axis}
        ...
        self.state.signal_buffer = ResampleBuffer(
            data=in_dat[:0], tvec=in_tvec[:0], template=template, last_update=time.time()
        )
    ...
    def _process(self, message: AxisArray) -> AxisArray:
        ...
        if self.settings.resample_rate is None and self.settings.axis not in self.state.signal_buffer.template.axes:
            buf = self.state.signal_buffer
        ...
        if ax_idx != 0:
            in_dat = np.moveaxis(in_dat, ax_idx, 0)
        ax = message.axes[self.settings.axis]
        in_tvec = ax.data if hasattr(ax, "data") else ax.value(np.arange(in_dat.shape[0]))
        buf.data = np.concatenate(
            (buf.data, in_dat), axis=0
        )
    ...
    def push_reference(self, message: AxisArray):
        ...
        if self.state.ref_axis[1] == 0:
            ...
            if hasattr(ax, "gain"):
                ...
                self.state.ref_axis = (self.state.ref_axis[0], self.state.ref_axis[1] + n_new)
            else:
                # Extend our time axis with the new data.
                new_tvec = np.concatenate((self.state.ref_axis[0].data, ax.data), axis=0)
                self.state.ref_axis = (
    ...
    def __next__(self):
        ...
        if self.settings.resample_rate is None:
            ...
            if hasattr(self.state.ref_axis[0], "data"):
                new_ref_ax = replace(self.state.ref_axis[0], data=self.state.ref_axis[0].data[ref_idx[-1] + 1:])
            else:
