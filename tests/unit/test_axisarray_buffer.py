...
def linear_axis_message():
    def _create(samples=10, channels=2, fs=100.0, offset=0.0):
        ...
        return AxisArray(data, dims=dims, axes=axes)
...
def test_add_and_get_linear(linear_axis_message):
    ...
    assert buf.n_samples == 20
    retrieved_msg = buf.get_data(15)
    assert retrieved_msg.shape == (15, 2)
    assert retrieved_msg.dims == msg1.dims
    # Last sample of msg2 is at 0.1 + 9*0.01 = 0.19
    # First sample of retrieved msg should be at 0.19 - 14*0.01 = 0.05
    assert retrieved_msg.axes["time"].offset == pytest.approx(0.05)
    expected_data = np.concatenate([msg1.data[5:], msg2.data])
...
def test_add_and_get_coordinate(coordinate_axis_message):
    ...
    expected_times = np.concatenate(
        [msg1.axes["time"].data[5:], msg2.axes["time"].data]
    )
